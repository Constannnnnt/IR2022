...
def search_contents_only(topic_number, hits):
    ...
    write_to_file("./pure_dense_search.txt", results)
...
def search_by_meta_criteria(topic, hits, searcher):
    ...
    write_to_file("./meta_criteria_dense_search_multi_fields.txt", results)

def dense_search(sparse_searcher, dense_searcher, topics):
    
    for topic in topics:
        topic_number = topic["num"]
        query = topic["query"]
        hits = dense_searcher.search(query, k = 30000, threads = 16)
...
def main():
    ...
    dense_search(sparse_searcher, dense_searcher, topics)
